...
| Command | Description |
|---|---|
| `codemap serve` | Run the [MCP server](/mcp) over stdio. `--profile agent\|core\|full` selects the [tool profile](/mcp#tool-profiles): `agent` is exactly 24 taught workflow tools plus `codemap_docs` (25 total), `core` preserves the compatible 25-tool surface, and default `full` exposes all 43. Same file < env (`CODEMAP_MCP_PROFILE`) < flag precedence as every other setting. |
