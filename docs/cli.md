...
| `codemap structural-manifest --json` | Emit the single-response `codemap.structural-manifest.v1` preflight for `export-symbols`: explicit export schema version, project identity, the exact export fingerprint, total records, completeness, and working-tree freshness counters plus the per-file delta (`changed_files`/`new_files`/`deleted_files`), so a consumer can decide what to re-ingest instead of re-reading the whole export. It streams indexed metadata without reading source bodies or loading the full export. |
...
| `codemap annotate --retarget <id> <sym> \| <from> <to>` | Repoint an existing annotation at a new target of the same kind — the repair for a note left dangling by a rename or removal. Reports `matched` for the new target. |
