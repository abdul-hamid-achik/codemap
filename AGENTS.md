...
(LSP + stdlib parsers, optional tree-sitter) with **semantic retrieval** (local veclite or
an explicit one-hop vecgrep CLI adapter) and
...
│                              #   `task action:test` runs its 107-assertion suite; dogfooded by
