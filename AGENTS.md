...
- **Tool profiles**: `CODEMAP_MCP_PROFILE=agent|core|full` (env) / `mcp.profile` (config file) /
  ...
  contract. The hermetic `BenchmarkProfileSchemaTax` measures 30,834 schema characters (≈7,709
  schema-approx-tokens) for agent/core versus 42,585 (≈10,647) for full on the current 43-tool
